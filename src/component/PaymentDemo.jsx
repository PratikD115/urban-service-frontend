...
const PaymentDemo = () => {
  ...
  const handleCardNumberChange = (event) => {
    ...
    const formattedCardNumber = newCardNumber.replace(/(.{4})/g, "$1 ").trim();
  ...
  const validateExpiryDate = () => {
    ...
    if (
      ...
      setExpiryDateError(
        "Card has expired. Please enter a future expiry date."
      );
    } else {
  ...
  const submithandler = async (data) => {
    try {
      ...
      const res = await axios.post(
        "http://localhost:4000/address/address",
        data
      );
      console.log(res.data);
      reset();
